    ...
    public class PlayroomManager_Runes
    {
        ...
        public void SpawnRunes_AdminCommand(Player invoker, Rune runeType, CustomRuneSpawn_Amount amount, CustomRuneSpawn_Position position, bool notifyOthers)
        {
            if(assignedPlayroom.matchState != MatchState.InGame) 
            {
                ...
                Misc_MessagingManager.SendMessageToTheClient("Can't spawn rune, match hasn't started", invoker.client.ch, MessageFromServer_WindowType.LightWindow, MessageFromServer_MessageType.Info);
                return;
            }
            int IntAmount;
            ...
            { 
                ...
                Misc_MessagingManager.SendMessageToTheClient("Can't spawn rune, no free rune spawns", invoker.client.ch, MessageFromServer_WindowType.LightWindow, MessageFromServer_MessageType.Error);
                return; 
            }
            
            if (IntAmount > freeSpawnsAmount) 
            {
                Misc_MessagingManager.SendMessageToTheClient($"Cant spawn precise amount of runes [{IntAmount}]. Had to cap max amount of runes because it was higher than amount of spawns[{freeSpawnsAmount}]", invoker.client.ch, MessageFromServer_WindowType.LightWindow, MessageFromServer_MessageType.Warning);
                IntAmount = freeSpawnsAmount; 
            }
